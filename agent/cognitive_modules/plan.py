...
def plan(name: str, world_context: str, observation: str, current_plan: str, reflections: str, reason_to_react: str, agent_bio: str = "", prompts_folder = "base_prompts_v0", changes_in_state: str = None, finetuning_recorder=None) -> tuple[str, str]:
    ...
    response = llm.completion(prompt=prompt_path, finetuning_recorder=finetuning_recorder, finetuning_key="plan", inputs=[name, world_context, observation, current_plan, reflections, reason_to_react, agent_bio, changes_in_state], system_prompt='plan_system_prompt.txt')
